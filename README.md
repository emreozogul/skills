...
Ships a **bundled asset vault** too: a stdlib-Python browser UI (`vault/vault.py`) to filter, preview, select, and download 147 license-verified packs — non-Kenney sources (KayKit, Quaternius, Synty, Poly Pizza, Poly Haven, ambientCG) now outnumber Kenney straight into your project — Kenney + ambientCG unzip in place (ambientCG even ships Godot `.tres` materials), Poly Haven pulls live thumbnails. **Free-first**: flags paid options with price and asks before assuming spend. Composes with `pixel-pipeline` (2D processing) and `godot` (engine import). Encodes a real lesson — *identify the source tool before approximating a style* — so the AI-pixel-art dead end isn't relearned.
