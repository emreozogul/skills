...
- **Beyond Kenney:** **KayKit** (CC0 — skeletons, dungeon, adventurers, animations), **Quaternius** (CC0 — nature, animals, monsters, characters), **Synty** (`$`), **Poly Pizza** (a low-poly model *search engine*), **Poly Haven**, **ambientCG** are all searchable. Honest caveat: those non-Kenney *model* sources gate downloads behind itch/Google-Drive/API-key, so in the vault they're **search + Open-page**; the **one-click downloads** are Kenney (zips), ambientCG (materials), and Poly Haven HDRIs. **Poly Pizza is now wired for live search + one-click `.glb` download** when a key is present — set `POLY_PIZZA_KEY` or drop the key in `vault/.pp_key` (gitignored, never committed). The UI then shows a live search bar over thousands of CC0/CC-BY low-poly models; results download straight into the vault. This is the true non-Kenney search engine. It has a **CC0-only** toggle if you want zero attribution hassle.
- **Attribution is handled for you:** every download appends to `<vault>/CREDITS.md` — provenance for CC0 (for the record) and the *legally-required* credit string for CC-BY. Ship that file with your build and you're covered; no scrambling for credits at release.
