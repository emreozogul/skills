...
| `references/3d.md` | **3D / low-poly mode**: `CharacterBody3D` (camera-relative) movement, a fixed-angle follow camera, `.glb` import + `AnimationPlayer`, `Area3D` Hitbox/Hurtbox, what game feel ports vs changes (hit-pause stays, shake→Camera3D, knockback→Vector3, flash→material, squash→model scale), `NavigationAgent3D` chase AI, and 3D gotchas. Core verified running on Godot 4.6. Pairs with `asset-pipeline`'s `lowpoly` packs. |
