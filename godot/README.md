...
| `references/3d.md` | **3D / low-poly mode**: `CharacterBody3D` (camera-relative) movement, a fixed-angle follow camera, `.glb` import + `AnimationPlayer`, `Area3D` Hitbox/Hurtbox, what game feel ports vs changes (hit-pause stays, shake→Camera3D, knockback→Vector3, flash→material, squash→model scale), `NavigationAgent3D` chase AI, and 3D gotchas. A `Player3D` + `FollowCamera3D` + `Chaser3D` slice is **verified running** on Godot 4.6 (chaser closes distance via group lookup, player lands on floor, `errors:[]`). Pairs with `asset-pipeline`'s `lowpoly` packs. |
